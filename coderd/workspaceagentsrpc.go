...
func (api *API) workspaceAgentRPC(rw http.ResponseWriter, r *http.Request) {
	...
	api.agentConnectionUpdate(closeCtx, workspaceAgent, build, conn)
...
func (api *API) agentConnectionUpdate(ctx context.Context,
	workspaceAgent database.WorkspaceAgent, workspaceBuild database.WorkspaceBuild,
	conn *websocket.Conn,
) {
	minder := &agentWebsocketMinder{
		apiCtx:            api.ctx,
		...
		logger:            api.Logger.With(slog.F("workspace_id", workspaceBuild.WorkspaceID)),
	}
	minder.init()
	go pprof.Do(ctx, pprof.Labels("agent", workspaceAgent.ID.String()), minder.sendPings)
	go pprof.Do(ctx, pprof.Labels("agent", workspaceAgent.ID.String()), minder.mind)
}
...
type agentWebsocketMinder struct {
	...
	// state manipulated by both sendPings() and mind() goroutines: needs to be threadsafe
	lastPing atomic.Pointer[time.Time]

	// state manipulated only by mind() goroutine: does not need to be threadsafe
...
func (m *agentWebsocketMinder) sendPings(ctx context.Context) {
...
func (m *agentWebsocketMinder) updateConnectionTimes(ctx context.Context) error {
...
func (m *agentWebsocketMinder) init() {
...
func (m *agentWebsocketMinder) mind(ctx context.Context) {
	defer func() {
